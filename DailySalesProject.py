...
for dts in daily_transactions:
    daily_transactions_split.append((dts.split(':')))
...
# separating the data
for tsc in transactions_clean:
    customers.append(tsc[0])
    sales.append(tsc[1])
    thread_sold.append(tsc[2])

# calculating total_sales

total_sales = 0
for s in sales:
    new_s = float(s.replace("$", ""))
    total_sales += new_s
print(total_sales)
...
# separating colors
for ts in thread_sold:
    if ts in {'white', 'red', 'blue', 'yellow', 'green', 'purple', 'black'}:
...
for c in colors:
    print("{} sold: {}".format(c.title(), color_count(c)))
